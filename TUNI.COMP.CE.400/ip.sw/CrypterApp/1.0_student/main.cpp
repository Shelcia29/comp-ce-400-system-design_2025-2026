...
SC_MODULE ( system_module )
{
	void app()
	{
		...
		while( true )
		{
			...
			sc_uint<32> feed = in_value.read();
			
			// Encryption: Permutation
        	encrypted_value = feed;
        	encrypted_value = (encrypted_value << 16) + (sc_uint<16>)(encrypted_value >> 16);

        	// Crypting with the key
        	encrypted_value = encrypted_value ^ 0xDEADBEEF;

        	// Decrypting with the key
        	decrypted_value = encrypted_value ^ 0xDEADBEEF;

        	// Undo the permutation
        	decrypted_value = (decrypted_value << 16) + (sc_uint<16>)(decrypted_value >> 16);

        	// Set the output value to the decrypted value
        	out_value.write(decrypted_value);
