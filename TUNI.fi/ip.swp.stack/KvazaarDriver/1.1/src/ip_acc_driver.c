...
// Sources: Cyclone V data sheet stating 72 as the base number for the first interrupt controller.
// Linux IRQ 72 for Cyclone V SoC Development Kit
#define IRQ_BASE 72
// Linux IRQ 28 for Cyclone V Arrow SoCkit
//#define IRQ_BASE 28
...
static void ip_acc_configuration(struct Accelerator* ip_acc_t)
{
	...
	printk(KERN_INFO DEVICE_NAME ": PIO IRQ ENABLE\n");
...
static int __init memory_init_module(void)
{
	...
	{
	int ret = request_irq(IRQ_BASE + RESULT_READY_IRQ, fpga_interrupt, IRQF_TRIGGER_RISING, "result_ready_irq", NULL);
	if (ret < 0)
	{
		printk(KERN_ALERT DEVICE_NAME ": Failure requesting irq %i, error %d\n", IRQ_BASE + RESULT_READY_IRQ, ret);
	}
	}
...
{	
	...
	{
	    //mode
	    uint32_t value = ip_acc_local->sad_result >> 24;
            copy_to_user(&values[0], &value, 4);
	    //values[0] = ip_acc_local->sad_result >> 24;
	    //cost 
            value = ip_acc_local->sad_result & 0xffffff;
            copy_to_user(&values[1], &value, 4);
	    //values[1] = ip_acc_local->sad_result & 0xffffff;
	}
	else if(*offset == 1)
	{
	    //mode
            uint32_t value = ip_acc_local->sad_result2 >> 24;
            copy_to_user(&values[0], &value, 4);
	    //values[0] = ip_acc_local->sad_result2 >> 24;
	    //cost
            value = ip_acc_local->sad_result2 & 0xffffff;
            copy_to_user(&values[1], &value, 4);
...
static ssize_t device_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
	...
	{
		copy_from_user(ip_acc_local->unfilt1_virt,values,len);
	...
	{
		copy_from_user(ip_acc_local->unfilt2_virt,values,len);
	...
	else if(ip_acc_local->location == 2)
	{
		for (y = 0; y < 64; ++y)
		{
		    copy_from_user(&ip_acc_local->orig_virt[y*64],&values[y*len],64);
		}
	...
	else if(ip_acc_local->location == 3)
	{
		...
		for(x = 0; x < len/4;x++)
		{
		    copy_from_user(ip_acc_local->config_virt,&configs[x],4);
	...
	else if(ip_acc_local->location == 4)
	{
		...
		uint32_t value;
		copy_from_user(&value,&configs[x],4);
		value |= 0x40000000;
		*ip_acc_local->config_virt = value;
